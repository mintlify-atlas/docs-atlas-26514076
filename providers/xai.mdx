---
title: 'xAI'
description: 'Grok models with extended thinking and structured output capabilities'
---
...
If you prefer not to process thinking content, you can disable it by setting the `thinking` option to `false`.
...
echo $response->text; // Final analysis
echo $response->additionalContent['thinking']; // Reasoning process
```
...
xAI supports streaming responses in real-time:
...
$stream = Prism::text()
    ->using(Provider::XAI, 'grok-4')
    ->withPrompt('Write a comprehensive guide to Laravel')
    ->asStream();
