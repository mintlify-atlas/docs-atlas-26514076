...
description: Extend your AI's capabilities by giving it access to functions it can call to check weather, search databases, or interact with APIs
---
...
For complete documentation on combining tools with structured output, see the [Structured output](./structured-output#combining-structured-output-with-tools) documentation.